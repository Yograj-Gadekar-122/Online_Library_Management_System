				...
				<div class="userContent">
					...
					//ACTIVITY PERFORM...
						...
						switch ($activity) {
							...
							case 'deleteBook':
                                	...
                                	$result = mysqli_num_rows(mysqli_query($con,"SELECT bookId FROM borrow Where bookId = '$deleteBookId'"));
                                	$availabilityBook = mysqli_num_rows(mysqli_query($con,"SELECT bookId FROM books WHERE available = '1' && bookId = '$deleteBookId'")) ;
                               
                                	if(empty($result) && !empty($availabilityBook)){
                                		$deleteResult = mysqli_query($con,"Delete From books Where bookId = '$deleteBookId'");
                            ...
                            case 'deleteReturnedBook':
                                	...
                                	$result = mysql_num_rows(mysqli_query($con,"SELECT returnId,returnDate FROM borrow Where returnId = '$deleteReturnId' && returnDate = $deleteReturnDate"));
                            ...
                            case 'updateBook':
                            		...
                            		if(!empty($uBookId)){
                            			...
                            			if($result['available'] == 1){
	                            			...
	                            			$result = mysqli_fetch_assoc(mysqli_query($con,"SELECT title,author,price,publisher FROM books WHERE bookId = '$uBookId'"));
	                            			?>
                            ...
                            case 'dBookRequest':
                                	...
                                	if(!empty($rd)){
                                		$deleteResult = mysqli_query($con,"Delete From requestforbooks Where requestDate = '$rd'");
					...
					//UPDATE BOOK...
						...
						if(isset($_REQUEST['updateBookBtn'])){
							...
							if(!empty($title) && !empty($author) && !empty($price) && !empty($publisher)){
								...
								$result = mysqli_query($con,"UPDATE books SET title = '$title', author = '$author', price = '$price', publisher = '$publisher' WHERE bookId = '$uBookId'");
	                ...
	                //Edit Admin...
	                    ...
	                    if(isset($_REQUEST['adminUpdateBtn'])){
	                        ...
	                        $query1 = mysqli_query($con,"UPDATE admin Set firstName ='$firstName', lastName ='$lastName', username ='$username', pwd ='$pwd', email ='$email', pic ='$actualFileName' Where id = '$uadminId'");
